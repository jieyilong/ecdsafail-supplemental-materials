...
\tikzset{
  ...
  add/.style={draw=black, line width=0.9pt, fill=addviolet,
              ...
              inner sep=4pt, align=center, font=\normalsize},
  stage/.style={draw=black, line width=0.9pt, fill=gateblue,
                minimum height=35mm, text width=45mm,
                rounded corners=1pt, inner sep=5pt,
                align=center, font=\small},
  stageviolet/.style={stage, fill=addviolet},
  stageorange/.style={stage, fill=lookorange},
...
% ============================================================================
% Page 1: Shor/ECDLP and windowed-addition background
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Background: Shor's algorithm for the elliptic-curve discrete logarithm};
  \node[font=\large] at (17.15,17.30)
    {Given a generator $P$ of prime order $r$ and $Q=[k]P$, recover the private scalar $k$};

  % Hidden-period reduction.
  \draw[black!45, line width=0.7pt, rounded corners=1pt]
    (1.0,10.75) rectangle (16.75,16.75);
  \node[font=\Large\bfseries, anchor=west] at (1.45,16.28)
    {1. Encode $k$ as a hidden period};
  \node[note, text width=14.4cm, align=left, anchor=north west,
        font=\normalsize] at (1.45,15.82)
    {Define a two-dimensional probe function over the elliptic-curve group:};
  \node[font=\Large] at (8.88,14.82)
    {$f(u,v)=[u]P+[v]Q=[u+kv]P.$};
  \node[note, text width=14.4cm, align=left, anchor=north west,
        font=\normalsize] at (1.45,14.20)
    {Because one additional copy of $Q$ equals $k$ copies of $P$,};
  \node[font=\Large] at (8.88,13.20)
    {$f(u-k,v+1)=f(u,v).$};
  \node[note, text width=14.4cm, align=left, anchor=north west,
        font=\normalsize] at (1.45,12.60)
    {Thus $(-k,1)$ generates the hidden period subgroup.  The double-scalar multiplication $[u]P+[v]Q$ is therefore the oracle whose periodicity encodes $k$; Fourier sampling reveals a vector orthogonal to that period.};

  % Top-level quantum routine.
  \draw[black!45, line width=0.7pt, rounded corners=1pt]
    (17.45,10.75) rectangle (33.30,16.75);
  \node[font=\Large\bfseries, anchor=west] at (17.90,16.28)
    {2. Coherent evaluation and Fourier sampling};
  \node[note, text width=14.5cm, align=left, anchor=north west,
        font=\normalsize] at (17.90,15.78)
    {Prepare two $t$-qubit scalar registers, with $M=2^t\gtrsim r$, and a point accumulator initialized to $\mathcal O$:};
  \node[font=\large] at (25.37,14.63)
    {$\displaystyle
      \frac{1}{M}\sum_{u,v=0}^{M-1}\ket{u,v}\ket{\mathcal O}
      \xrightarrow{\ U_f\ }
      \frac{1}{M}\sum_{u,v=0}^{M-1}\ket{u,v}\ket{[u]P+[v]Q}.$};
  \node[note, text width=14.5cm, align=left, anchor=north west,
        font=\normalsize] at (17.90,13.63)
    {Apply the two-register Fourier transform (or its semiclassical implementation), measure $(s,t)$, and classically recover and verify};
  \node[font=\Large] at (25.37,12.50)
    {$t\equiv ks\pmod r,\qquad k\equiv ts^{-1}\pmod r,\qquad [k]P\stackrel{?}{=}Q.$};
  \node[note, text width=14.5cm, align=center, font=\small] at (25.37,11.45)
    {The Fourier convention may reverse a sign; the recoverable modular relation is unchanged in substance.};

  % Windowed implementation of U_f.
  \draw[black!45, line width=0.7pt, rounded corners=1pt]
    (1.0,0.72) rectangle (33.30,10.10);
  \node[font=\Large\bfseries, anchor=west] at (1.45,9.66)
    {3. Evaluate $U_f$ by coherent windowed point additions};
  \node[font=\large] at (17.15,8.70)
    {$\displaystyle
      u=\sum_{j=0}^{\ell-1} u_j2^{jw},\qquad
      v=\sum_{j=0}^{\ell-1} v_j2^{jw},\qquad
      \ell=\left\lceil\frac{n}{w}\right\rceil.$};
  \node[font=\large] at (17.15,7.22)
    {$\displaystyle
      [u]P+[v]Q
      =\sum_{j=0}^{\ell-1}[u_j]P_j
       +\sum_{j=0}^{\ell-1}[v_j]Q_j,
      \qquad P_j=[2^{jw}]P,\quad Q_j=[2^{jw}]Q.$};
  \node[note, text width=29.5cm, align=center, font=\normalsize] at (17.15,6.42)
    {For each coherent $w$-bit address $z\in\{u_j,v_j\}$, a QROM table $T_j$ supplies the selected precomputed point, the point is added to the accumulator, and the lookup workspace is uncomputed:};
  \node[font=\large] at (17.15,5.40)
    {$\displaystyle
      \ket{z}\ket{0}_A\ket{R}
      \xrightarrow{\mathrm{lookup}}
      \ket{z}\ket{T_j[z]}_A\ket{R}
      \xrightarrow{\mathrm{add}}
      \ket{z}\ket{T_j[z]}_A\ket{R+T_j[z]}
      \xrightarrow{\mathrm{unlookup}}
      \ket{z}\ket{0}_A\ket{R+T_j[z]}.$};
  \node[note, text width=29.5cm, align=center, font=\normalsize] at (17.15,4.48)
    {The window register is preserved as a coherent address; it is not merely a single CCX-style control bit.  For $n=256$ and $w=16$, there are $\ell=16$ windows per scalar and $32$ conceptual point-addition updates before boundary-level reductions.};

  \node[detailgreen, text width=28.8cm, minimum height=18mm,
        font=\normalsize] at (17.15,2.24)
    {\textbf{Scope of this deck and submission \texttt{8e9c9a2}.}
     The analyzed source implements the in-place mixed-addition body
     $R\mapsto R+A$ for one classically known, temporarily materialized addend
     $A=(x_A,y_A)$.  Under the challenge evaluator, it reaches
     $Q=1{,}151$ peak logical qubits and $T=1{,}299{,}453$ average executed
     Toffoli gates, giving $S=Q\times T=1{,}495{,}670{,}403$.
     It does not implement the scalar registers, Fourier transforms, or QROM
     lookup/unlookup shown above.  Embedding it into a fully coherent windowed-Shor
     circuit therefore requires a separately specified and validated
     lookup--use--unlookup interface for the selected point.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 2: full ECDLP circuit with the U_f window wiring expanded
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
...
% ============================================================================
% Page 3: generic in-place point addition and its Shor-window interpretation
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
...
% ============================================================================
% Page 4: inside one point-addition box
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {From the point-addition algorithm to the \texttt{8e9c9a2} submission};
  \node[font=\large] at (17.15,17.35)
  ...
  \node[font=\large\bfseries, anchor=west] at (2.35,3.78)
    {Source stages in submission \texttt{8e9c9a2} (same columns as the pipeline)};
...
% ============================================================================
% Page 5: stage 1, coordinate differences
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 1: form the coordinate differences};
  ...
  \node[lookup, text width=38mm] (UX) at (20.0,15.55)
    {unload classical bits\\$\ket{x_A}_{C_x}\mapsto\ket{0}^{\otimes256}$};
  \draw[flow] (18.0,15.55) -- (UX.west);
  \node[note, anchor=south west] at (29.0,13.05) {$X=\ket{d_x}$};
  ...
  \node[lookup, text width=38mm] (UY) at (20.0,10.65)
    {unload classical bits\\$\ket{y_A}_{C_y}\mapsto\ket{0}^{\otimes256}$};
  \draw[flow] (18.0,10.65) -- (UY.west);
  \node[note, anchor=south west] at (29.0,8.15) {$Y=\ket{d_y}$};

  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,17.75)
    {\textbf{Stage 1 optimization stack}\\[0.8mm]
     {\footnotesize off-peak classical addend $\bullet$ serialized $256$-qubit workspace\\
     $53$-bit pseudo-Mersenne fold $\bullet$ $19$-bit final comparison\\
...
% ============================================================================
% Page 6: stage 1 implementation details
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 1 detail: optimized coordinate subtraction};
  ...
  \node[lookup, text width=35mm] (LX4) at (19.0,13.95)
  ...
  \node[lookup, text width=35mm] (LY4) at (19.0,11.55)
  ...
  \node[detail, text width=90mm, minimum height=59mm, font=\small] (C2) at (17.15,6.55)
    {{\normalsize\bfseries Pseudo-Mersenne modular correction}\\[1.2mm]
     For $0\le x,y<p$, computing $r_0$ generates internal borrows $b_1,\ldots,b_{255}$ and the final borrow $b=b_{256}=[y<x]$:
     {\normalsize
     \[
       r_0=(y-x)\bmod 2^{256}
       =\begin{cases}
     ...
     \[
       ...
       =\begin{cases}
        ...
        \end{cases}
     \]}
     Thus $r=r_0-bF\pmod{2^{256}}$. The source corrects $\operatorname{low}_{53}(r_0)$ and measurement-uncomputes $b$ using a $19$-MSB phase predicate. Equal top windows can depend on lower bits; these truncations are benchmark-validated rather than all-input proofs.};

  \node[detailviolet, text width=82mm, minimum height=59mm, font=\small] (C3) at (27.6,6.55)
    {{\normalsize\bfseries Measurement-based carry uncomputation}\\[-0.3mm]
     {\itshape ``carry venting'' in the implementation}\\[1.2mm]
     ...
     {\normalsize
     \[
       \sum_{q_1,q_2}\alpha_{q_1q_2}\ket{q_1,q_2}\ket{0}
       \longmapsto
       \sum_{q_1,q_2}\alpha_{q_1q_2}\ket{q_1,q_2}\ket{q_1q_2}.
     \]}
     Measuring the ancilla in the $X$ basis with result $m\in\{0,1\}$ leaves
     {\normalsize
     \[
       \sum_{q_1,q_2}\alpha_{q_1q_2}(-1)^{m q_1q_2}\ket{q_1,q_2}.
     \]}
     The correction $\mathrm{CZ}^{m}_{q_1,q_2}$ removes the phase exactly, omitting the reverse Toffoli. The final $b$ remains live through $-bF$ and is cleaned separately.};

  \node[detailgreen, text width=296mm, minimum height=16mm] at (17.15,1.55)
    {\textbf{Interpretation.} Stage 1 does not introduce a new point-addition identity. Its savings come from source-level liveness control, arithmetic specialized to $p$, and measurement-assisted uncomputation. The classical loading shown here is valid for the challenge's fixed-addend interface; a coherent windowed-Shor implementation instead requires a separately validated QROM lookup/use/unlookup construction.};

  \node[note, text width=29cm, align=center, font=\small\itshape] at (17.15,0.38)
...
% ============================================================================
% Page 7: stage 2, dialog-GCD division
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: compute the slope by dialog-GCD division};
  ...
  \node[detailviolet, text width=47mm] (SW) at (5.3,7.02)
    {prepare the Bezout pair\\[0.8mm]
     $Z$ is a clean $256$-qubit ancilla\\
     $(Y,Z):(d_y,0)\longmapsto(0,d_y)$\\
     $Y$ is cleared to receive $\lambda$};
  ...
  \node[detailorange, text width=50mm] (TAPE) at (16.0,11.1)
    {base-$5$ dialog codec\\[0.7mm]
     $\tau$: quantum transcript register\\
     storing compressed Jump-2 symbols};
  ...
  \draw[flow] (WALK.east) -- node[note, above] {live step controls}
    (20.0,13.5) -- (APPLY.north);
  ...
  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,16.95)
    {\textbf{Stage 2 optimization stack}\\[0.8mm]
     {\footnotesize Jump-2 $\bullet$ base-$5$ dialog codec $\bullet$ streamed replay\\
     shrinking active widths $\bullet$ comparison narrowing\\
     temporary-lane reuse $\bullet$ scheduled carry venting}};

  \node[note, anchor=north east] at (32.0,12.55)
    {$X=\ket{d_x}$ (restored)};
  \node[note, anchor=south west] at (29.6,8.7) {$Y=\ket{\lambda}$};
  \node[note, anchor=south west] at (29.2,5.35)
    {$Z=\ket{0}^{\otimes256}$ (clean and released)};

  \draw[black!50, line width=0.7pt, rounded corners=1pt]
    (2.0,1.05) rectangle (32.3,3.92);
  \node[font=\large\bfseries, anchor=west] at (2.4,3.52)
    {Physical-register map and cleanup};
  \node[font=\normalsize] at (17.15,2.78)
    {$\ket{p}_u\ket{d_x}_X\ket{0}_Y\ket{d_y}_Z
      \xrightarrow{\ \text{forward walk, live update, and reverse cleanup}\ }
      \ket{0}_u\ket{d_x}_X\ket{\lambda}_Y\ket{0}_Z,\qquad
      \lambda=d_y d_x^{-1}\pmod p$};
  \node[note, text width=29cm, align=center] at (17.15,1.82)
    {During the forward walk, physical register $X$ serves as the Euclidean operand $v$ and is temporarily modified; the reverse walk restores $X=d_x$.  Physical registers $(Y,Z)$ are the coefficient pair and evolve from $(0,d_y)$ to $(\lambda,0)$.  The displayed endpoint is the ideal accepted-path relation for the fixed $261$-step truncated schedule validated on the benchmark support.};
  \node[note, text width=29cm, align=center, font=\small\itshape] at (17.15,0.70)
    {Source correspondence: \texttt{mod\_mul\_inverse\_in\_place(..., Direction::Inverse)}, \texttt{forward\_gcd\_jump}, \texttt{reverse\_gcd\_jump}, and \texttt{DialogCodec}; related source paths use the ``K5 codec'' jargon for the five-symbol dialog-codec family.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 8: Jump-2 step used inside stage 2
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2 detail: the Jump-2 Euclidean step};
  \node[font=\large] at (17.15,17.05)
    {Compute division by following the binary-GCD path of $(p,d_x)$ and updating the numerator coherently};
  \node[font=\normalsize] at (17.15,16.25)
    {$\alpha p+\beta d_x=1\ \Longrightarrow\ \beta\equiv d_x^{-1}\pmod p
      \ \Longrightarrow\ \lambda=d_y\beta\equiv d_y/d_x\pmod p$};

  \node[detailviolet, text width=35mm, minimum height=23mm] (S0) at (3.3,13.4)
    {active state\\[1mm]
     $u$ odd, $v\geq0$\\
     coefficient pair $(Y,Z)$};
  ...
  \draw[black!50, line width=0.7pt, rounded corners=1pt]
    (1.25,6.10) rectangle (13.3,10.65);
  \node[font=\large\bfseries, anchor=west] at (1.7,10.18)
    {Jump-2 in one equation};
  \node[font=\normalsize] at (7.25,9.15)
    {$\displaystyle
      v\longmapsto
      \begin{cases}
        v/2, & v/2\text{ is odd},\\
        v/4, & v/2\text{ is even}.
      \end{cases}$};
  \node[note, text width=10.8cm, align=center] at (7.25,8.12)
    {After halving, compare $u$ and $v$, optionally swap, and subtract.  The symbol
     $\sigma=(b,s,s_2)$ records subtraction, swapping, and the second halving, respectively.};
  \node[note, text width=10.8cm, align=center] at (7.25,7.50)
    {Only five values occur: $s_2=0\Rightarrow b=1$ and $b=0\Rightarrow s=0$.};
  \foreach \x/\sym/\fillcol in {
      2.45/{(1,0,0)}/gateblue,
      4.80/{(1,1,0)}/addviolet,
      7.15/{(0,0,1)}/lookorange,
      9.50/{(1,0,1)}/outgreen,
  ...
  \node[font=\large\bfseries, anchor=west] at (14.55,10.18)
    {The same decisions construct the quotient};
  \node[note, text width=17.6cm, align=left, anchor=west] at (14.55,9.13)
    {Every halving, swap, and subtraction is a linear update $M$ of the Euclidean pair $(u,v)$.  The same live controls apply the corresponding coefficient update $M^{-T}$ to $(Y,Z)$.  Thus the GCD walk reduces $(p,d_x)$ toward $(1,0)$ while the paired update turns the numerator $d_y$ into $d_y d_x^{-1}=\lambda$.};
  \node[font=\normalsize] at (23.55,7.48)
    {$\text{walk: }(u,v)\xrightarrow{\ M\ }(u',v')
      \qquad\Longleftrightarrow\qquad
      \text{physical pair: }(Y,Z)\xrightarrow{\ M^{-T}\ }(Y',Z')$};

  \node[note, text width=30cm, align=center] at (17.15,4.85)
    {Jump-2 packages the mandatory halving, an optional second halving, and the subsequent operand-order/subtraction choice into one recorded instruction.  It therefore makes more Euclidean progress per transcript symbol than a one-halving schedule.};
  \node[note, text width=30cm, align=center] at (17.15,3.35)
    {Because the choices depend on the quantum value $d_x$, they may be entangled with the input and cannot be measured or discarded.  Submission \texttt{8e9c9a2} stores 261 coherent symbols, uses their live controls to form $\lambda$, and replays them backward to restore $X=d_x$ and clean the workspace.};
  \node[note, text width=30cm, align=center, font=\small\itshape] at (17.15,1.65)
...
% ============================================================================
% Page 9: base-5 codec and streamed replay used inside stage 2
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2 detail: base-$5$ codec and streamed dialog replay};
  \node[font=\large] at (17.15,17.28)
    {Store the coherent five-way GCD decisions near their information-theoretic minimum};
  ...
  \node[note, text width=8.0cm, align=center] at (5.75,11.28)
    {$\phi$ is implemented as a reversible basis-state permutation: no decision is measured, copied classically, or discarded.};

  \draw[black!50, line width=0.7pt, rounded corners=1pt]
    (11.2,10.65) rectangle (22.5,15.95);
  \node[font=\large\bfseries] at (16.85,15.42) {2. Pack three symbols};
  \node[note, text width=9.8cm, align=center] at (16.85,14.50)
    {A naive encoding stores each Jump-2 symbol in three qubits.  Because only five values occur, each symbol can instead be regarded as one base-$5$ digit};
  \node[font=\normalsize] at (16.85,13.55)
    {$d_j=\phi(\sigma_j)\in\{0,1,2,3,4\}$};
  \node[note, text width=9.8cm, align=center] at (16.85,12.92)
    {Three such digits can be packed as};
  \node[font=\normalsize] at (16.85,12.30)
    {$z=d_0+5d_1+25d_2$};
  \node[font=\normalsize] at (16.85,11.68)
    {$0\leq z\leq5^3-1=124<128=2^7$};
  \node[font=\normalsize] at (16.85,11.08)
    {$3\cdot3=9
      \quad\longrightarrow\quad
      \left\lceil\log_2(5^3)\right\rceil=7\text{ qubits}$};
  ...
  \node[detailviolet, text width=39mm, minimum height=18mm] (FWD) at (3.3,7.5)
    {forward GCD walk\\produces $\sigma_0,\sigma_1,\ldots$};
  \coordinate (BRANCH) at (6.1,7.5);
  \node[detailviolet, text width=43mm, minimum height=18mm] (LIVE) at (9.5,9.0)
    {use live symbol immediately\\for $M^{-T}$ Bezout update};
  \node[detailgreen, text width=34mm, minimum height=18mm] (LAMBDA) at (15.0,9.0)
    {accumulate slope\\$Y\longrightarrow\lambda$};
  \node[detailorange, text width=40mm, minimum height=18mm] (ENC) at (9.5,6.0)
    {encode each full group\\and release two wires};
  \node[detailgreen, text width=40mm, minimum height=18mm] (TAU) at (15.0,6.0)
    {$\tau$: quantum transcript register\\
     compressed GCD decisions};
  \node[detailorange, text width=40mm, minimum height=18mm] (DEC) at (20.5,6.0)
    {decode one group\\only when replay reaches it};
  \node[detailgreen, text width=54mm, minimum height=18mm] (REVPLAY) at (27.3,6.0)
    {reverse the GCD walk\\consume $\tau$; restore $X=d_x$;\\clear Euclidean workspace};
  \draw[flow] (FWD.east) -- (BRANCH);
  \draw[flow] (BRANCH) |- node[note, above, near end] {live controls} (LIVE.west);
  \draw[flow] (LIVE.east) -- (LAMBDA.west);
  \draw[flow] (BRANCH) |- node[note, below, near end] {same controls} (ENC.west);
  \draw[flow] (ENC.east) -- (TAU.west);
  \draw[flow] (TAU.east) -- (DEC.west);
  \draw[flow] (DEC.east) -- (REVPLAY.west);

  \draw[black!50, line width=0.7pt, rounded corners=1pt]
    (1.1,1.3) rectangle (33.2,4.85);
  \node[font=\large\bfseries, anchor=west] at (1.55,4.38)
    {Compression and scheduling jointly reduce peak width};
  ...
  \node[note, text width=29cm, align=center] at (17.15,1.72)
    {$\tau$ is a quantum transcript entangled with $d_x$, not a classical log.  As the active Euclidean registers narrow, the encoded tape grows into their released space; reverse replay decodes only the current group, consumes its symbols, restores the Euclidean workspace, and returns the transcript ancillas to $\ket{0}$.};
  \node[note, text width=30cm, align=center, font=\small\itshape] at (17.15,0.62)
    {Source terminology: this slide describes \texttt{DialogCodec::Pair} and \texttt{DialogCodec::Triple}; related source paths and development notes use the ``K5 codec'' jargon for the corresponding five-symbol encoding idea.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 10: width scheduling and temporary reuse used inside stage 2
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2 detail: width scheduling and qubit reuse};
  \node[font=\large] at (17.15,16.98)
...
% ============================================================================
% Page 11: stage 3, prepare X workspace
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 3: prepare the $X$ workspace};
  \node[font=\large] at (17.15,17.28)
  ...
  \node[registerbox, text width=45mm] at (20.0,6.95)
    {$Y$ bypasses this stage unchanged};
  \node[note, anchor=south west] at (29.0,6.95)
    {$Y=\ket{\lambda}$ (unchanged)};

  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,17.75)
    {\textbf{Stage 3 optimization stack}\\[0.8mm]
     {\footnotesize classical $3x_A\bmod p$ $\bullet$ transient $256$-qubit materialization\\
     $53$-bit fold $\bullet$ $19$-bit comparison $\bullet$ immediate unload}};
  ...
  \node[note, text width=29cm, align=center] at (17.15,2.38)
    {The following stage subtracts $\lambda^2$, producing $x_R+2x_A-\lambda^2=x_A-x_{R'}$.  The submitted route uses its configured low-cost modular-adder variant; the classical multiplier does not occupy quantum storage outside this stage.};
...
% ============================================================================
% Page 12: stage 4, specialized square
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 4: subtract the specialized modular square};
  \node[font=\large] at (17.15,17.28)
  ...
  \node[detailorange, text width=40mm] (SPLIT) at (6.2,15.0)
    {logical split\\[1mm]
     $\lambda=L+2^{128}H$\\
     $L,H\in\{0,1\}^{128}$};
  \node[note, anchor=south west] at (29.0,15.0)
    {$Y=\ket{\lambda}$ (unchanged)};
  ...
  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,17.75)
    {\textbf{Stage 4 optimization stack}\\[0.8mm]
     {\footnotesize three-square Karatsuba $\bullet$ symmetric squaring $\bullet$ streamed products\\
     pseudo-Mersenne NAF fold $\bullet$ from-zero adds $\bullet$ adaptive carry venting}};
  ...
  \node[note, text width=29cm, align=center, font=\small\itshape] at (17.15,0.75)
    {Source correspondence: \texttt{mod\_square\_sub\_pm\_secp256k1\_symmetric}, including the $C$, $A$, and $B$ build/apply/unbuild stages.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 13: stage 4 detail, symmetry-aware streamed Karatsuba square
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 4 detail: symmetry-aware streamed Karatsuba squaring};
  \node[font=\large] at (17.15,17.28)
    {A standard one-level Karatsuba identity is specialized to squaring, reversible streaming, and the secp256k1 prime.};

  % Algebra panel.
  \draw[black!55, line width=0.8pt, rounded corners=1pt]
    (1.25,6.15) rectangle (16.90,16.50);
  \node[font=\large\bfseries, anchor=west] at (1.70,15.98)
    {Standard Karatsuba square identity};
  \node[note, text width=14.4cm, align=center] at (9.08,14.92)
    {Let $\beta=2^{128}$ and write $\lambda=L+\beta H$.  To avoid confusion with the selected elliptic-curve addend $A$, denote the three temporary squares by $U$, $V$, and $W$:};
  \node[font=\large] at (9.08,13.88)
    {$U=L^2,\qquad V=H^2,\qquad W=(L+H)^2.$};
  \node[font=\large] at (9.08,12.65)
    {$W-U-V=2LH,\qquad
      \lambda^2=U+\beta(W-U-V)+\beta^2V.$};
  \node[font=\large] at (9.08,11.42)
    {$p=2^{256}-F,\quad F=2^{32}+977
      \quad\Longrightarrow\quad \beta^2=2^{256}\equiv F\pmod p.$};
  \node[font=\large] at (9.08,10.08)
    {$\therefore\quad
      \lambda^2\equiv U+\beta(W-U-V)+FV\pmod p.$};
  \node[note, text width=14.2cm, align=center] at (9.08,9.12)
    {Karatsuba supplies the algebra; the circuit-specific gains come from symmetric partial products, immediate accumulation, and pseudo-Mersenne folding.};
  ...
  \node[detail, text width=137mm, minimum height=9mm] at (25.20,14.72)
    {\textbf{1. Split and prepare.} View $Y=\lambda$ as the $128$-bit slices $L$ and $H$; reversibly build the $129$-bit sum $S=L+H$.};
  \node[detailviolet, text width=137mm, minimum height=9mm] at (25.20,13.20)
    {\textbf{2. Sum square.} Build $W=S^2$, apply $-\beta W$ directly to $X$, and reverse the squarer to return its product register to zero.};
  \node[detail, text width=137mm, minimum height=9mm] at (25.20,11.68)
    {\textbf{3. Low-half square.} Build $U=L^2$, apply $-U+\beta U$ directly to $X$, and unbuild $U$.};
  \node[detailviolet, text width=137mm, minimum height=9mm] at (25.20,10.16)
    {\textbf{4. High-half square.} Build $V=H^2$, apply $+\beta V-FV$ directly to $X$, and unbuild $V$.};
  ...
  \node[font=\large\bfseries, anchor=west] at (1.70,5.13)
    {Circuit-level specializations beyond standard Karatsuba};
  \node[detail, text width=63mm, minimum height=24mm] at (5.10,3.15)
    {\textbf{Symmetric squaring}\\[1mm]
     Diagonal bits are copied directly.  The three squares form $24{,}512$ unordered cross products, $24.9\%$ fewer than a direct $256$-bit symmetric square.};
  \node[detailgreen, text width=63mm, minimum height=24mm] at (13.15,3.15)
    {\textbf{Direct streamed accumulation}\\[1mm]
     Only one of $U$, $V$, or $W$ is live at a time.  Each signed contribution is applied to $X$ and immediately uncomputed, avoiding a persistent $512$-bit square.};
  \node[detailorange, text width=63mm, minimum height=24mm] at (21.20,3.15)
    {\textbf{Pseudo-Mersenne fold}\\[1mm]
     The high limb $2^{256}V$ becomes $FV$ modulo $p$, replacing generic full-width modular reduction by a fixed-constant fold.};
  \node[detailviolet, text width=63mm, minimum height=24mm] at (29.25,3.15)
    {\textbf{Sparse signed constant}\\[1mm]
     $F=2^{32}+2^{10}-2^6+2^4+1$, so $FV$ is assembled from five shifted additions or subtractions rather than a generic multiplication.};

  \node[note, text width=31cm, align=center, font=\small\itshape] at (17.15,0.55)
    {The Karatsuba identity is exact; any benchmark-calibrated finite-width correction is a separate optimization.  Source correspondence: \texttt{mod\_square\_sub\_pm\_secp256k1\_symmetric}.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 14: stage 5, forward multiplication via dialog-GCD replay
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 5: forward multiplication via dialog-GCD replay};
  \node[font=\large] at (17.15,17.28)
    {$Y=\lambda\longmapsto\lambda(x_A-x_{R'})\pmod p$ while restoring $X=x_A-x_{R'}$};
  ...
  \node[detailviolet, text width=48mm] (WALK2) at (9.3,13.5)
    {forward Euclidean reduction\\(Jump-2 GCD walk on $(p,X)$)\\[1mm]
     temporarily transform $X=v$;\\record the reversible transcript $\tau$};
  \node[lookup, text width=35mm] (PREG2) at (9.3,15.85)
    {initialize\\$u\gets p$};
  \draw[wire] (PREG2.south) -- (WALK2.north);

  \node[detailorange, text width=50mm] (TAPE2) at (14.8,11.1)
    {$\tau$: quantum transcript register\\[0.7mm]
     stores the compressed symbols\\
     for $X=x_A-x_{R'}$};
  \draw[flow] (WALK2.south east) -- (TAPE2.north west);

  % In Direction::Forward, this clean lane is allocated only after the walk.
  \node[lab, anchor=east] at (14.1,5.35) {$Z:\ \ket{0}^{\otimes256}$};
  \draw[wire] (14.25,5.35) -- (32.2,5.35);
  \node[detailviolet, text width=52mm] (SW2) at (16.7,7.02)
    {prepare the multiplicand pair\\[0.8mm]
     allocate clean $256$-qubit ancilla $Z$ after the walk\\
     $(Y,Z):(\lambda,0)\longmapsto(0,\lambda)$};
  \draw[wire] (16.7,8.7) -- (SW2.north);
  \draw[wire] (16.7,5.35) -- (SW2.south);

  \node[detailviolet, text width=50mm] (REPLAY2) at (23.2,7.02)
    {decode $\tau$ in reverse and apply\\the forward-transpose updates\\[1mm]
     $\ket{0}_Y\ket{\lambda}_Z\mapsto
      \ket{\lambda(x_A-x_{R'})}_Y\ket{0}_Z$};
  \draw[flow] (TAPE2.south east) -- (REPLAY2.north west);
  \draw[wire] (23.2,8.7) -- (REPLAY2.north);
  \draw[wire] (23.2,5.35) -- (REPLAY2.south);

  \node[detailgreen, text width=43mm] (CLEAN2) at (28.5,13.5)
    {reverse Euclidean walk\\[1mm]
     consume $\tau$; restore $X$;\\
     clear Euclidean workspace};
  \draw[flow] (TAPE2.north east) -- (CLEAN2.south west);

  \node[note, anchor=south west] at (13.0,13.5)
    {after the walk: $X/v=\ket{0}$ on convergent inputs};

  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,16.95)
    {\textbf{Stage 5 optimization stack}\\[0.8mm]
     {\footnotesize shared Jump-2 engine $\bullet$ fresh base-$5$ dialog\\
     in-place forward multiply $\bullet$ streamed reverse replay\\
     shrinking widths $\bullet$ per-call carry schedules}};

  \node[note, anchor=north east] at (32.0,12.55)
    {$X=\ket{x_A-x_{R'}}$ (restored)};
  \node[note, anchor=south west] at (28.6,8.7)
    {$Y=\ket{\lambda(x_A-x_{R'})}$ (when convergent)};
  \node[note, anchor=south west] at (29.1,5.35)
    {$Z=\ket{0}^{\otimes256}$ (clean and released)};

  \draw[black!50, line width=0.7pt, rounded corners=1pt]
    (2.0,0.42) rectangle (32.3,4.72);
  \node[font=\large\bfseries, anchor=west] at (2.4,4.37)
    {Why dialog-GCD implements multiplication in Stage 5};
  \node[note, anchor=west, font=\small] at (2.55,3.78)
    {Stage 2 realizes the in-place division unitary};
  \node[font=\large] at (9.25,3.24)
    {$U_{\mathrm{div}}:\ \ket{X}\ket{Y}
      \longmapsto \ket{X}\ket{YX^{-1}\bmod p}.$};
  \node[note, anchor=west, font=\small] at (2.55,2.66)
    {Reversibility therefore gives};
  \node[font=\large] at (9.25,2.12)
    {$U_{\mathrm{div}}^{-1}:\ \ket{X}\ket{Y}
      \longmapsto \ket{X}\ket{YX\bmod p}.$};
  \node[note, text width=13.2cm, align=center, font=\small] at (9.25,1.48)
    {Thus Stage 5 reuses the low-space division construction in reverse; it does not compute $X^{-1}$.};
  \node[note, text width=13.6cm, align=left, anchor=north west, font=\small]
    at (17.55,3.87)
    {Here $X=x_A-x_{R'}$.  Stage 2's transcript described the different operand $d_x=x_R-x_A$ and was consumed during cleanup.  Stage 5 therefore constructs a fresh $\tau(X)$, whose reverse replay supplies the $X$-dependent coefficient updates that transform $\lambda$ into $\lambda X$ while restoring $X$.};
  \node[detailgreen, text width=13.6cm, minimum height=9mm, align=left,
        anchor=north west, font=\scriptsize] at (17.55,2.62)
    {\textbf{Resource effect.} The in-place map reuses $X$, $Y$, $Z$, and $\tau$, avoiding the additional product register and cleanup required by a straightforward out-of-place modular multiplication.  This lowers peak-width pressure, but does not establish a Toffoli advantage: transcript replay remains a dominant gate cost.  Cleanup is exact; the product relation requires $261$-step convergence, validated only on the benchmark support.};
  \node[note, text width=29cm, align=center, font=\scriptsize\itshape] at (17.15,0.16)
    {Source correspondence: \texttt{mod\_mul\_inverse\_in\_place(..., Direction::Forward)}, \texttt{ITERS=261}, and reverse replay with \texttt{apply\_step\_forward}.};
\end{tikzpicture}

\newpage

% ============================================================================
% Page 15: stage 6, recover affine output
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);

  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 6: recover the affine output coordinates};
  \node[font=\large] at (17.15,17.28)
  ...
  \node[note, anchor=south west] at (29.0,13.0) {$Y=\ket{y_{R'}}$};
  ...
  \node[detail, anchor=north east, text width=70mm, minimum height=12mm] at (32.0,17.75)
    {\textbf{Stage 6 optimization stack}\\[0.8mm]
     {\footnotesize transient classical coordinates $\bullet$ fused complement-plus-add subtraction\\
     measurement-based carry cleanup $\bullet$ $53$-bit fold $\bullet$ $19$-bit comparison}};
