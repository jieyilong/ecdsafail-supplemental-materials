...
% Restrained functional palette: sufficiently distinct for navigation while
% remaining pale enough that equations and register states stay dominant.
\definecolor{gateblue}{RGB}{241,246,251}
\definecolor{lookorange}{RGB}{252,247,238}
\definecolor{invviolet}{RGB}{247,244,250}
\definecolor{outgreen}{RGB}{241,248,243}
\definecolor{warnred}{RGB}{252,244,244}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[warning,text width=278mm,minimum height=31mm] at (17.15,5.25)
    {\textbf{Evidential status.} An isolated replay of all $9{,}024$ challenge tests reports
     zero classical, phase, and ancilla
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {What ``Shrunken-PZ'' means};
  \node[font=\large] at (17.15,17.15)
    {A low-width reversible organization of Proos--Zalka-style modular inversion};

  \node[detail,text width=91mm,minimum height=58mm] at (6.1,12.9)
    {\textbf{1. Mathematical task}\\[1mm]
     For $x\ne0\pmod p$, an extended Euclidean computation produces a coefficient $t$ such that
     \[
       tx+sp=\gcd(x,p)=1,
       \qquad t\equiv x^{-1}\pmod p.
     \]
     The point-addition circuit uses this map to compute
     $\lambda=d_y d_x^{-1}\bmod p$.};

  \node[inversion,text width=91mm,minimum height=58mm] at (17.15,12.9)
    {\textbf{2. Why it is ``shrunken''}\\[1mm]
     During the Euclidean recurrence, the remainders become shorter while the B\'ezout
     coefficients grow. Rather than reserve separate full-width registers for every quantity,
     the implementation tracks their current lengths and packs nonoverlapping values into
     shared fixed-width banks.};

  \node[result,text width=91mm,minimum height=58mm] at (28.2,12.9)
    {\textbf{3. The implementation used here}\\[1mm]
     Submission \texttt{b6f2b0a} uses the register-shared descendant of this architecture:
     two $259$-qubit work banks, compact length/control metadata, and a fixed schedule of
     $1{,}479$ reversible steps. Dynamic cursors address only the active packed fields.};

  \draw[flow] (10.95,12.9) -- (12.25,12.9);
  \draw[flow] (22.05,12.9) -- (23.35,12.9);

  \node[detailwide,text width=279mm,minimum height=38mm] at (17.15,7.55)
    {\textbf{Packed state in \texttt{b6f2b0a}.} One bank contains the evolving coefficient,
     quotient, and remainder $(t,q,r)$; the other contains $(t',r')$. Their source-enforced
     live-width bounds are
     \[
       \operatorname{bitlen}(t)+1+\operatorname{bitlen}(q)+\operatorname{bitlen}(r)\le259,
       \qquad
       \operatorname{bitlen}(t')+\operatorname{bitlen}(r')\le259.
     \]
     Reusing the space vacated by shrinking values is the central width-saving mechanism.};

  \node[warning,text width=279mm,minimum height=32mm] at (17.15,3.35)
    {\textbf{Not dialog-GCD.} ``Shrunken-PZ'' is project shorthand for this low-width
     Proos--Zalka-style EEA lineage. The Q825 circuit carries the complete packed Euclidean
     state and reverses the scheduled updates to clean it; it does \emph{not} store a compressed
     branch transcript for later dialog-GCD replay.};
\end{tikzpicture}
\newpage
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Why the register-shared core peaks at 825 qubits in Stage 2 and 5};
  \node[font=\large] at (17.15,17.15)
  ...
  \fill[lookorange] (21.4,12.6) rectangle (29.65,14.9);
  \fill[outgreen] (29.65,12.6) rectangle (32.6,14.9);
  \foreach \xx in {11.55,21.4,29.65}{\draw[black,line width=0.7pt] (\xx,12.6)--(\xx,14.9);}
  \node[font=\large,align=center] at (6.63,13.75) {$W_1$\\$259$ lanes};
  \node[font=\large,align=center] at (16.48,13.75) {$W_2$\\$259$ lanes};
  \node[font=\large,align=center] at (25.53,13.75) {passenger $d_y$\\$257$ lanes};
  \node[font=\normalsize,align=center] at (31.13,13.75) {metadata\\$37$};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 1: fold the 257th lane instead of retaining a reducer};
  \node[font=\large] at (17.15,17.15)
...
% ============================================================================
% Algorithmic background for Stage 2
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: Algorithm behind shrunken-PZ modular inversion};
  \node[font=\large] at (17.15,17.15)
    {Restoring long division inside a reversible extended-Euclidean schedule};

  % Pseudocode panel.
  \draw[line width=0.9pt,rounded corners=1pt]
    (0.75,0.25) rectangle (33.55,16.45);
  \node[anchor=west,font=\Large\bfseries] at (1.15,15.95)
    {Register-shared modular inverse};
  \draw[black!55,line width=0.6pt] (1.0,15.45)--(33.3,15.45);
  \node[anchor=west,font=\large,text width=30.8cm,align=left] at (1.15,14.95)
    {\textbf{Input:} $x\in\{1,\ldots,p-1\}$ for the odd prime $p$.
     \quad \textbf{Output:} $z=x^{-1}\bmod p$; all work registers are restored.};

  \node[anchor=west,font=\large\bfseries] at (1.15,13.85) {1.};
  \node[anchor=west,font=\large] at (2.05,13.85)
    {$x_0\gets\min(x,p-x)$; \quad $\pi\gets[x>p/2]$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,13.85)
    {$\triangleright\ x_0\equiv(-1)^\pi x\pmod p$};

  \node[anchor=west,font=\large\bfseries] at (1.15,12.65) {2.};
  \node[anchor=west,font=\large] at (2.05,12.65)
    {$(t,r)\gets(1,p)$; \quad $(t',r')\gets(0,x_0)$; \quad $q\gets0$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,12.65)
    {$\triangleright\ $ initialize the two packed Euclidean pairs};

  \node[anchor=west,font=\large\bfseries] at (1.15,11.45) {3.};
  \node[anchor=west,font=\large] at (2.05,11.45)
    {\textbf{while} $r'\neq0$ \textbf{do}};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,11.45)
    {$\triangleright\ \gcd(r,r')=1$};

  \node[anchor=west,font=\large\bfseries] at (1.15,10.25) {4.};
  \node[anchor=west,font=\large] at (3.15,10.25)
    {$q\gets0$; \quad $j\gets\operatorname{bitlen}(r)-\operatorname{bitlen}(r')$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,10.25)
    {$\triangleright\ $ begin one restoring division};

  \node[anchor=west,font=\large\bfseries] at (1.15,9.05) {5.};
  \node[anchor=west,font=\large] at (3.15,9.05)
    {\textbf{for} $j,j-1,\ldots,0$ \textbf{do}};

  \node[anchor=west,font=\large\bfseries] at (1.15,7.85) {6.};
  \node[anchor=west,font=\large] at (4.25,7.85)
    {\textbf{if} $r\ge 2^j r'$ \textbf{then} $r\gets r-2^j r'$; \quad $q\gets q+2^j$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,7.85)
    {$\triangleright\ q=\lfloor r_{\rm in}/r'\rfloor$};

  \node[anchor=west,font=\large\bfseries] at (1.15,6.65) {7.};
  \node[anchor=west,font=\large] at (3.15,6.65)
    {$t'\gets t'+qt$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,6.65)
    {$\triangleright\ $ consume the same quotient bits in reverse order};

  \node[anchor=west,font=\large\bfseries] at (1.15,5.45) {8.};
  \node[anchor=west,font=\large] at (3.15,5.45)
    {$((t,r),(t',r'))\gets((t',r'),(t,r))$; \quad $\pi\gets\pi\oplus1$.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,5.45)
    {$\triangleright\ $ advance to the next Euclidean pair};

  \node[anchor=west,font=\large\bfseries] at (1.15,4.25) {9.};
  \node[anchor=west,font=\large] at (2.05,4.25)
    {Continue inactive padding until the fixed $1{,}479$-microstep schedule ends.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,4.25)
    {$\triangleright\ $ input-independent reversible runtime};

  \node[anchor=west,font=\large\bfseries] at (1.15,3.05) {10.};
  \node[anchor=west,font=\large] at (2.05,3.05)
    {$z\gets(-1)^{\pi+1}t'\pmod p$; use $z$, then reverse Steps 2--9.};
  \node[anchor=east,font=\normalsize,text=black!65] at (32.95,3.05)
    {$\triangleright\ r=1,\ r'=0,\ xz\equiv1\pmod p$};

  % Arithmetic and packing invariants connect the pseudocode to the next slides.
  \draw[black!55,line width=0.6pt,rounded corners=1pt,fill=gateblue]
    (1.15,0.55) rectangle (33.0,2.35);
  \draw[black!35,line width=0.5pt] (17.1,0.72)--(17.1,2.18);
  \node[anchor=west,font=\normalsize,align=left,text width=14.9cm] at (1.5,1.45)
    {\textbf{Euclidean invariants}\quad
     $rt+r't'=p$,\\[-0.5mm]
     $r\equiv(-1)^{\pi+1}xt'$,\quad
     $r'\equiv(-1)^\pi xt\pmod p$.};
  \node[anchor=west,font=\normalsize,align=left,text width=14.7cm] at (17.45,1.45)
    {\textbf{Packed-state invariants}\quad
     $W_1=t\Vert q\Vert r$,\quad
     $W_2=\operatorname{rot}_{\ell_s}(t'\Vert r')$,\\[-0.5mm]
     $\ell_t+1+\ell_q+\operatorname{bitlen}(r)\le n+3$,\quad
     $\operatorname{bitlen}(t')+\ell_{r'}\le n+3$.\\[-0.5mm]
     {\scriptsize Here $n=256$ is the \texttt{secp256k1} field-element bit width;
     $\operatorname{rot}_{\ell_s}$ cyclically rotates the packed layout by $\ell_s$ lanes and is not an arithmetic shift.}};
\end{tikzpicture}
\newpage

% ============================================================================
% Stage 2 implementation overview
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
  ...
  \node[detail,text width=90mm,minimum height=51mm] at (17.15,6.2)
    {\textbf{Register-shared core}\\[1mm]
     ...
     phase, sign, and iteration bits control the reversible microsteps. The cyclic rotation aligns
     the active slices of $t'$ and $r'$ with the current arithmetic window without allocating shifted copies.};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: the packed Euclidean state machine};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: reflected input and a fixed reversible schedule};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: register sharing and dynamic width};
  \node[font=\large] at (17.15,17.15)
    {The principal low-$Q$ gain comes from assigning physical lanes by lifetime rather than by logical register name.};

  \node[font=\large\bfseries,anchor=west] at (2.0,15.55)
    {Naive logical layout: five separate worst-case allocations};
  ...
  \draw[black,line width=1pt,fill=lookorange] (2.0,9.3) rectangle (6.6,10.9);
  \draw[black,line width=1pt,fill=invviolet] (6.6,9.3) rectangle (9.0,10.9);
  \node[font=\normalsize,align=center] at (4.3,10.1)
    {$t$ grows\\[-0.5mm]$1\longrightarrow p$};
  \node[font=\footnotesize,align=center] at (7.8,10.1)
    {$q$ temporary\\[-0.5mm]$0\to q_i\to0$};
  \node[font=\normalsize,align=center] at (12.4,10.1)
    {$r$ shrinks\\[-0.5mm]$p\longrightarrow1$};
  ...
  \node[font=\normalsize,align=center] at (21.75,10.1)
    {$t'$ grows\\[-0.5mm]$0\longrightarrow t'_{\mathrm{term}}$};
  \node[font=\normalsize,align=center] at (28.65,10.1)
    {$r'$ shrinks\\[-0.5mm]$x_0\longrightarrow0$};
  \node[font=\normalsize,anchor=south] at (25.4,11.0) {$W_2$ (259 lanes, rotated by $\ell_s$)};

  \node[note,text width=29.5cm,align=center,font=\normalsize] at (17.15,8.55)
    {$x_0=\min(x,p-x)$. At termination,
     $z=(-1)^{\pi+1}t'_{\mathrm{term}}\bmod p=x^{-1}\bmod p$;
     each quotient $q_i$ is assembled, consumed, and cleared within one restoring-division round.};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: dynamic cursors avoid copied position registers};
  \node[font=\large] at (17.15,17.15)
  ...
  \node[detail,text width=91mm,minimum height=50mm] at (17.15,5.75)
    {\textbf{Rotation cursor}\\[1mm]
     ...
     allocating shifted copies of either operand. This is a reversible permutation of storage lanes,
     not multiplication by $2^{\ell_s}$; inverse rotation restores the original packed layout.};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: three disjoint Q826 host windows inherited by Q825};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: omit one persistent $\ell_q$ bit in the Q826 $\rightarrow$ Q825 transition};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: consuming an omitted quotient-length bit};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: producing $\lambda$ while preserving $d_x$ and $d_y$};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: measurement-based release and later recomputation};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 2: inherited low-$Q$ optimization stack};
  \node[font=\large] at (17.15,17.15)
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stages 3--4: coordinate update and alternate-witness construction};
  \node[font=\large] at (17.15,17.15)
    {The same $X$ and $Y$ registers are recycled from input coordinates to output coordinates.};

  \node[gate,text width=78mm,minimum height=28mm] (Z) at (6.2,12.2)
    {erase $d_y$\\$Y\gets d_y-\lambda d_x=0$};
  \node[gate,text width=78mm,minimum height=28mm] (NX) at (17.15,12.2)
    {form new $x$ coordinate\\$X\gets\lambda^2-x_R-x_A=x_{R'}$};
  \node[gate,text width=78mm,minimum height=28mm] (ND) at (28.1,12.2)
    {construct the alternate witness\\$X\gets x_A-x_{R'}=d'_x$\\$Y\gets\lambda d'_x=d'_y=y_{R'}+y_A$};
  ...
  \node[detail,text width=88mm,minimum height=53mm] at (27.3,6.1)
    {\textbf{Alternate witness for Stage 5}\\[-1mm]
     The coordinate identity $y_{R'}=\lambda(x_A-x_{R'})-y_A$ gives
     \[
       d'_y=\lambda d'_x=y_{R'}+y_A.
     \]
     Thus $d'_y(d'_x)^{-1}=\lambda$, providing the independent witness used to erase the slope in Stage 5.};
\end{tikzpicture}
\newpage
...
% ============================================================================
% Stage 6 output recovery
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
  \node[font=\LARGE\bfseries] at (17.15,18.0)
    {Stage 6: recover the output and clear remaining workspace};
  \node[font=\large] at (17.15,17.15)
    {The final off-peak stage converts the alternate witness into the affine coordinates of $R'=R+A$.};

  \node[inversion,text width=59mm,minimum height=30mm] (S0) at (4.0,12.5)
    {input after Stage 5\\
     $X=d'_x=x_A-x_{R'}$\\
     $Y=d'_y=y_{R'}+y_A$\\
     EEA state and $\lambda$ cleared};
  \node[classical,text width=63mm,minimum height=30mm] (SX) at (12.6,12.5)
    {recover the $x$ coordinate\\
     $X\gets x_A-X=x_{R'}$\\
     transiently load $x_A$};
  \node[classical,text width=63mm,minimum height=30mm] (SY) at (21.2,12.5)
    {recover the $y$ coordinate\\
     $Y\gets Y-y_A=y_{R'}$\\
     transiently load $y_A$};
  \node[result,text width=56mm,minimum height=30mm] (SO) at (29.3,12.5)
    {output in place\\
     $\ket{x_{R'}}_X\ket{y_{R'}}_Y$\\
     temporary lanes clean};
  \draw[flow] (S0.east)--(SX.west);
  \draw[flow] (SX.east)--(SY.west);
  \draw[flow] (SY.east)--(SO.west);

  \node[detail,text width=88mm,minimum height=52mm] at (6.2,6.15)
    {\textbf{Algebraic recovery}\\[-1mm]
     The alternate witness satisfies
     \[
       d'_x=x_A-x_{R'},\qquad
       d'_y=\lambda d'_x=y_{R'}+y_A.
     \]
     The two subtractions therefore recover $x_{R'}$ and $y_{R'}$ without allocating separate output registers.};
  \node[detail,text width=88mm,minimum height=52mm] at (17.15,6.15)
    {\textbf{Transient classical-coordinate access}\\[-1mm]
     The classically known coordinates of $A$ are loaded only for their corresponding subtraction and are
     removed immediately afterward. The same $X$ and $Y$ lanes that held the Euclidean witnesses become the
     final output registers.};
  \node[result,text width=88mm,minimum height=52mm] at (28.1,6.15)
    {\textbf{Off-peak reversible teardown}\\[-1mm]
     Stage 6 reaches $Q=789$, below the two $Q=825$ EEA peaks. The transient load, carry, and padding lanes are
     returned clean, leaving only the API-width affine coordinates of $R'$.};

  \node[note,text width=284mm,minimum height=18mm] at (17.15,1.45)
    {Source correspondence: \texttt{ec3.alt.new\_x\_restore} and
     \texttt{ec3.alt.new\_y\_restore}.};
\end{tikzpicture}
\newpage
