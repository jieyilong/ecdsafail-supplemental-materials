...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[detail, text width=90mm, minimum height=59mm, font=\small] (C2) at (17.15,6.55)
    {{\normalsize\bfseries Pseudo-Mersenne modular correction}\\[1.2mm]
     For canonical field inputs $0\le x,y<p$, let $b=[y<x]$ be the subtraction borrow. The $256$-bit subtractor returns
     {\normalsize
     \[
       r_0=(y-x)\bmod 2^{256}
       =\begin{cases}
     ...
     \[
       ...
       =\begin{cases}
        ...
        \end{cases}
     \]}
     Thus $r=r_0-bF\pmod{2^{256}}$.
     The source corrects only $\operatorname{low}_{53}(r_0)$ and reconstructs $b$ from $\operatorname{MSB}_{19}(y)$ and $\operatorname{MSB}_{19}(x)$; both truncations are benchmark-validated rather than all-input proofs.};
