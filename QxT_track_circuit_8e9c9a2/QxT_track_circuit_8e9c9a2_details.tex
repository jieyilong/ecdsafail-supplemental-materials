...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[detailorange, text width=82mm, minimum height=49mm] (C1) at (6.7,6.55)
    {\textbf{Classical-addend specialization}\\[1.5mm]
     In submission \texttt{8e9c9a2}, $(x_A,y_A)$ is classically known. Each coordinate is copied into $C$ with classically controlled $X$ gates, used by one reversible subtraction, and XOR-unloaded before $C$ is released. Thus the two $256$-qubit copies are never simultaneously live.};

  \node[detail, text width=82mm, minimum height=49mm] (C2) at (17.15,6.55)
    {\textbf{Pseudo-Mersenne modular correction}\\[1.5mm]
     For \texttt{secp256k1},
     \[
       p=2^{256}-F,\qquad F=2^{32}+977.
     \]
     The overflow or borrow correction can therefore be folded through the low $53$ bits rather than by a second full-width reduction. The final conditional correction compares a selected $19$-bit most-significant window.};

  \node[detailviolet, text width=82mm, minimum height=49mm] (C3) at (27.6,6.55)
    {\textbf{Vented carry cleanup}\\[1.5mm]
     The adder stores selected carries in temporary ancillas during the forward pass, then removes them by $X$-basis measurement and conditional phase feedback:
     \[
       \ket{c}\ \xrightarrow{\ H,\,M_X\ }\ m,
       \qquad \mathrm{CZ}^{m}\ \text{restores the phase}.
     \]
     Each vent exchanges one temporary qubit for one fewer carry-uncompute Toffoli, which is favorable in this off-peak phase.};
...
% ============================================================================
% Page 12: phase 5, reverse dialog-GCD multiplication
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
...
% ============================================================================
% Page 13: phase 6, recover affine output
% ============================================================================
\begin{tikzpicture}[x=1cm,y=1cm]
  \path[use as bounding box] (0,0) rectangle (34.3,18.6);
