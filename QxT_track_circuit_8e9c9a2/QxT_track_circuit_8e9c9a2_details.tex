...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[note, text width=17.6cm, align=left, anchor=west] at (14.55,9.13)
    {The walk applies a data-dependent linear update $M$ to $(u,v)$, where $v$ is physically the coordinate register $X$.  The paired arithmetic applies $M^{-T}$ directly to physical registers $(Y,Z)$, preserving the division relation.  The bit $s_2$ selects whether this update contains one or two powers of two, while $b$ and $s$ select subtraction and operand order.};
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[note, text width=29cm, align=center] at (17.15,1.72)
    {As the active Euclidean registers narrow, $\tau$ grows into the released space. Streamed decoding consumes its symbols during reverse replay, restores the Euclidean workspace, and returns the transcript ancillas to $\ket{0}$.};
